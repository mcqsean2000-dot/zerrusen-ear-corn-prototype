...
- `functions/src/notification-reconciliation.js` and the Firebase runtime provide a separate, bounded ten-minute recovery schedule for pending/retryable jobs that predate enablement and `processing` leases older than 15 minutes. Lease recovery is transactional, preserves the attempt count, and terminates exhausted jobs. It requires both delivery and reconciliation flags, and has not been deployed or enabled.
...
- Firebase schedule-handler tests cover disabled runs, trusted schedule-time conversion, idempotent retry results, malformed events, and static trigger configuration.
