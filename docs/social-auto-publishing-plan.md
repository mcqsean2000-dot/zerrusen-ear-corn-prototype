...
This foundation is deployed and connected to a five-minute Firebase schedule, but has no committed credentials and cannot publish while the runtime gate remains disabled. A separate ten-minute schedule recovers expired publishing leases only when its own reconciliation gate is enabled. The admin page loads a review-only weekly batch, requires explicit approval before queueing, and exposes guarded `needs_reconciliation` controls.
