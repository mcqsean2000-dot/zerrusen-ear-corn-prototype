...
Production status verified on 2026-08-24: the five-minute publisher and ten-minute reconciliation schedules are deployed and active. The publisher has enabled Secret Manager bindings for the Facebook Page ID, Instagram account ID, and Page access token; secret values remain outside the repository. Both `SOCIAL_PUBLISHING_ENABLED` and `SOCIAL_RECONCILIATION_ENABLED` are enabled. The publisher deployment timestamp immediately follows the merge of PR #81, which added the Instagram media-processing wait. Production still uses Meta Graph API `v25.0`, while the reviewed repository preflight requires exactly `v26.0`; resolve and verify that configuration drift before the controlled Instagram retry. Closure also requires sanitized provider-ID and terminal queue evidence from that retry, plus confirmation of token-renewal ownership and the weekly human-review procedure.
