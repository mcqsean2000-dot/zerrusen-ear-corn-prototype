...
- `functions/src/firebase-notification-delivery-handler.js` - SDK-free Firestore event adapter with explicit enablement and retry signaling
...
- `functions/src/notification-reconciliation.js` - bounded recovery dispatcher for pending, retryable, and expired processing jobs
- `functions/src/social-post-queue.js` - SDK-free validation boundary for human-approved Facebook and Instagram queue records
...
- Authenticated admin API routes can list social posts in `needs_reconciliation` and record an audited publish, retry-confirmed-safe, or skip resolution. Visible admin controls for this workflow are not implemented yet.
