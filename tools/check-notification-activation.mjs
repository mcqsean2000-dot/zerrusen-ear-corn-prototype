...
function preflightError(code) {
  const error = new Error("Notification activation preflight could not run.");
  error.code = code;
  return error;
}
...
async function main() {
  if (process.argv.length !== 3) {
    throw preflightError("notification_env_path_required");
  }
  ...
  if (
    ...
    throw preflightError("notification_env_path_invalid");
  }

  const result = validateDisabledNotificationConfig(
...
main().catch((error) => {
  const code = String(error && error.code || "notification_activation_preflight_failed")
    .replace(/[^a-z0-9_/-]/gi, "_");
  console.error(`Notification activation preflight failed (${code}).`);
  process.exitCode = 1;
});
